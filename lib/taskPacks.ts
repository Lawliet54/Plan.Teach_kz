...
const fallbackTestDefinitions = [
  {
    ...
    options: [
      ...
      "Шамалардың мағынасын тексермей формуланы бірден қолдану",
    ],
  },
  {
  ...
  },
  {
    ...
    options: [
      "Берілген шамалардың бірліктерін тексермей есепті қайта шығару",
      "Қате шыққан нәтижені бірден дұрыс деп қабылдау",
  ...
  },
  {
    ...
    options: [
      "Бір формула барлық физикалық жағдайды дәл сипаттайды",
      "Өлшем бірлігін тексеру нәтиженің дұрыстығына әсер етпейді",
      "Модельдің қолданылу шарттарын тексерудің қажеті жоқ",
...
function fallbackItems(pack: TaskPack): TaskPackItem[] {
  ...
  return [
    ...
    ...calculations,
    {
      ...
      prompt: `${pack.title} бойынша виртуалды немесе қолжетімді құралдармен зерттеу жүргізіңіз. Бір тәуелсіз параметрді кемінде үш рет өзгертіңіз, қалған маңызды параметрлерді тұрақты ұстаңыз, өлшеу кестесін жасаңыз, қажет болса графикті сипаттаңыз, қателік көзін атаңыз және дерекке сүйенген қорытынды жазыңыз.`,
      instruction: "Төмендегі 2D зертханаға өтуге болады. Жауапта құралдар, параметрлер, кемінде 3 өлшеу, қажет болса график сипаттамасы, қателік көзі және қорытынды болуы керек.",
