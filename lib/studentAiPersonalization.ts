...
function buildProgressAdvice(progress?: StoredAdaptiveProgress | null) {
  if (!progress) {
    return "Бұл тақырып бойынша әлі нәтиже жоқ. Алдымен теорияны оқып, жеңіл тапсырмалардан бастаған дұрыс.";
  }
  ...
  if (percent < 50) {
    return attempts > 1
      ? "Бұл тақырыпта әлі қиындық бар. Анықтаманы, формуланы және қате кеткен тапсырмаларды қайта қарап шыққан дұрыс."
      : "Соңғы нәтиже төмен. Қазір күрделі есепке өтпей, алдымен анықтама → формула → жеңіл мысал ретімен қайталау керек.";
  }

  if (percent < 70) {
    return attempts > 1
      ? "Нәтиже әлі 70%-ға жеткен жоқ. Қате кеткен сұрақтарды талдап, терминдер мен формуланың мағынасын нақтылап алған дұрыс."
      : "Негізгі ойдың бір бөлігі түсінікті, бірақ тақырып толық меңгерілмеген. Қысқа қайталау жасап, тапсырманы қайта орындау ұсынылады.";
  }

  if (percent < 90) {
    return "Тақырыптың негізгі бөлігі түсінікті. Енді қате кеткен тапсырмаларды қарап, формула мен негізгі ұғымдарды бекіту керек.";
  }

  return "Нәтиже жақсы. Тақырыпты бекіту үшін ұқсас есеп немесе қысқа қайталау орындауға болады.";
}
